...
def _batched_device_put_impl(
  ...
  ys = []  # type: ignore
  shard_arg_indexes, shard_arg_xs, shard_arg_shardings = [], [], []
...
def _device_put_transpose(cts, *_, devices, srcs):
  ...
  dp_indices, dp_args, dp_devices, dp_srcs = [], [], [], []
  for i, (ct, device, src) in enumerate(zip(cts, devices, srcs)):
    if type(ct) is not ad.Zero:
      dp_indices.append(i)
      dp_args.append(ct)
      dp_devices.append(device)
      dp_srcs.append(src)
  if dp_args:
    ys = device_put_p.bind(*dp_args, devices=dp_srcs, srcs=dp_devices)
    for i, y in zip(dp_indices, ys):
      results[i] = y
  return results
...
def _tpu_gpu_device_put_lowering(ctx, *xs, devices, srcs):
  def lower(x, device, src):
    if (isinstance(device, (Sharding, TransferToMemoryKind)) and
      ...
      return x
    return x
  return [lower(x, d, s) for x, d, s in zip(xs, devices, srcs)]
