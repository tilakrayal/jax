...
#define NYI(msg)                            \
  ...
  LogicalResult infer(vector::ShapeCastOp op) {
    ...
    // E.g. we shouldn't try adding a minor implicit dim when folding lanes.
    ...
    if (res_shape.size() >= 2 &&
      ...
      setLayout(op, VectorLayout(layout.bitwidth(), layout.offsets(),
                             native_tiling, layout.implicit_dim()),
