...
struct MultiDimReductionBitwidthConvert
  ...
  LogicalResult matchAndRewrite(vector::MultiDimReductionOp op,
    ...
    auto acc_ty = dyn_cast<VectorType>(op.getAcc().getType());
...
// Rewrite `vector.shape_cast` of a `truncf` op into a `shape_cast` of the
// `truncf` op's input. This addresses the "only 32-bit shape casts supported"
// error.
class ShapeCastOfTruncf final : public OpRewritePattern<vector::ShapeCastOp> {
 public:
  using OpRewritePattern::OpRewritePattern;

  LogicalResult matchAndRewrite(vector::ShapeCastOp op,
                                PatternRewriter &rewriter) const override {
    auto truncf = op.getSource().getDefiningOp<arith::TruncFOp>();
    if (truncf == nullptr) {
      return rewriter.notifyMatchFailure(op, "source is not a truncf op");
    }
    auto ity = dyn_cast<VectorType>(truncf.getIn().getType());
    auto oty = dyn_cast<VectorType>(op.getType());
    if (!ity || !oty) {
      return rewriter.notifyMatchFailure(op,
                                         "input/output is not a vector type");
    }
    if (!oty.getElementType().isBF16()) {
      return rewriter.notifyMatchFailure(op, "output is not bf16");
    }
    rewriter.replaceOpWithNewOp<arith::TruncFOp>(
        op, oty,
        rewriter.create<vector::ShapeCastOp>(
            op.getLoc(), VectorType::get(oty.getShape(), ity.getElementType()),
            truncf.getIn()));
    return success();
  }
};
...
struct LinalgVectorizationPass
  ...
  void runOnOperation() override {
    ...
    patterns.add<ShapeCastOfTruncf>(ctx);
    ...
    func.walk([&](Operation *op) {
      if (dyn_cast<linalg::LinalgOp>(op) ||
          ...
          dyn_cast<vector::ShapeCastOp>(op)) {
